...
class HarnessMonitor:
    ...
    def _init_config(self):
        ...
        # 告警阈值
        self.alert_thresholds = self.config.get("alert_thresholds", {"response_time_ms": 5000, "error_rate": 0.05, "memory_usage_percent": 80})
    ...
    def record_security_event(self, event_type: SecurityEventType, severity: str, component: str, description: str, details: Dict[str, Any], action_taken: str) -> str:
        """
        记录安全事件
        ...
        event = SecurityEvent(
            event_id=event_id, timestamp=datetime.now(), event_type=event_type, severity=severity, component=component, description=description, details=details, action_taken=action_taken
        )
    ...
    def record_metrics(self, component: str, response_time_ms: float, throughput_rps: float, error_rate: float, resource_usage: Dict[str, float], custom_metrics: Dict[str, Any] = None) -> str:
        """
        记录性能指标
    ...
    def _check_alert_thresholds(self, metrics: PerformanceMetrics):
        ...
        if alerts:
            self.record_security_event(
                ...
                details={"alerts": alerts, "metrics": metrics.to_dict()},
    ...
    def get_security_events(self, event_type: Optional[SecurityEventType] = None, severity: Optional[str] = None, resolved: Optional[bool] = None, limit: int = 100) -> List[SecurityEvent]:
        """获取安全事件"""
        with self._lock:
    ...
    def get_performance_metrics(self, component: Optional[str] = None, time_range_minutes: Optional[int] = None, limit: int = 100) -> List[PerformanceMetrics]:
        """获取性能指标"""
        with self._lock:
    ...
    def generate_report(self, report_type: str = "summary", time_range_minutes: int = 60) -> Dict[str, Any]:
        """生成监控报告"""
        cutoff_time = datetime.now().timestamp() - (time_range_minutes * 60)

        with self._lock:
            # 筛选时间范围内的数据
            recent_decisions = [d for d in self.decision_traces if d.timestamp.timestamp() > cutoff_time]
            recent_events = [e for e in self.security_events if e.timestamp.timestamp() > cutoff_time]
            recent_metrics = [m for m in self.performance_metrics if m.timestamp.timestamp() > cutoff_time]

        # 计算统计信息
        decision_stats = {"total": len(recent_decisions), "by_outcome": {}, "avg_confidence": 0}
        ...
        if recent_metrics:
            ...
            metric_stats = {"avg_response_time_ms": avg_response_time, "avg_error_rate": avg_error_rate, "total_metrics": len(recent_metrics)}
        ...
        report = {
            ...
            "summary": {"system_health": self._calculate_system_health(event_stats, metric_stats), "recommendations": self._generate_recommendations(event_stats, metric_stats)},
        }
