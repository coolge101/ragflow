...
import type { ShellNavItem } from "../constants/adminAccess";
import { useAuth } from "../context/AuthContext";
import { useNarrowLayout } from "../hooks/useNarrowLayout";

export type ShellChromeProps = {
  brandSubtitle: string;
  /** Already filtered via `filterNavItems` in the parent layout. */
  navItems: ShellNavItem[];
  headerExtra?: ReactNode;
};

export function ShellChrome({ brandSubtitle, navItems, headerExtra }: ShellChromeProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const narrow = useNarrowLayout();
  const [mobileNavOpen, setMobileNavOpen] = useState(false);
  const { me, loading, error, contractWarning, refresh } = useAuth();
