...
if not any("/tbox" in str(p) for p in client_urls_prefix):
    logging.warning("TBOX routes not registered (no /v1/tbox prefix in blueprints). web-tbox needs `api/apps/tbox_app.py` in the running image or PYTHONPATH from this repo.")
else:
    logging.info("TBOX extension registered (/%s/tbox)", API_VERSION)
