...
class UserService(CommonService):
    ...
    def query_user(cls, email, password):
        ...
        """
        # Prefer oldest row when duplicate emails exist (legacy DBs before unique index).
        user = (
            cls.model.select()
            .where(
...
class UserTenantService(CommonService):
    ...
    @classmethod
    @DB.connection_context()
    def get_all_members_for_tenant(cls, tenant_id):
        """All valid memberships for tenant including owner row (for TBOX managed-user UI)."""
        fields = [
            cls.model.id,
            cls.model.user_id,
            cls.model.status,
        ...
        return list(
            cls.model.select(*fields).join(User, on=((cls.model.user_id == User.id) & (cls.model.status == StatusEnum.VALID.value))).where(cls.model.tenant_id == tenant_id).dicts(),
        )
