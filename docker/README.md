...
The [.env](./.env) file contains important environment variables for Docker. If your clone has no `.env` (common when `.gitignore` excludes it), copy the tracked template and edit:

```bash
cp .env.example .env
```

See also **[`docs/TBOX_DEPLOY_RUNBOOK.md`](../docs/TBOX_DEPLOY_RUNBOOK.md)** §3.1.
