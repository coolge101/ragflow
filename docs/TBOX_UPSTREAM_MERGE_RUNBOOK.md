...
`post_upstream_merge` / `pre_release` 后 §5 自动化可能已通过，但 **Phase 16–17 UI 仍为 ☐**。与 **`tbox_post_upstream_merge.sh`** 收尾、**[`TBOX_DEPLOY_RUNBOOK.md`](./TBOX_DEPLOY_RUNBOOK.md) §8.1** · Walkthrough **[§2.1](./TBOX_UI_ACCEPTANCE_WALKTHROUGH.md)** · VM 验收 **§5** 文案一致：

| 顺序 | 操作 | 说明 |
|------|------|------|
| 1 | `bash scripts/tbox_phase16_17_handtest.sh` | bundle + C/D 清单（Walkthrough **步骤 D** 5180 准生产前置与 **C §7** 共用） |
| 2 | 浏览器 Walkthrough **步骤 C §7 + D** on 5180 | Citation + 检索高亮 |
...
# 5180：Walkthrough 步骤 D 5180 准生产前置 + C §7 + D；可选 /review/step/phase16-17
bash scripts/tbox_phase16_17_finish.sh --archive
```

**同一链亦见于**：**[`TBOX_QUICKSTART.md`](./TBOX_QUICKSTART.md) §1.3** · **`bash scripts/tbox_host_check.sh`** · **`bash scripts/tbox_pre_release.sh --help`** · **`tbox_print_release_next_steps.sh`**

详 **[`TBOX_DEPLOY_RUNBOOK.md`](./TBOX_DEPLOY_RUNBOOK.md)** §8.1 · **[`TBOX_SMOKE_SCRIPTS.md`](./TBOX_SMOKE_SCRIPTS.md)** · **[`TBOX_SMOKE_ENV.md`](./TBOX_SMOKE_ENV.md)** · **`scripts/tbox_smoke.env.example`**。
...
| 项 | 值 |
|----|-----|
| HEAD | `8e3ebc572` — TBOX Phase 66: SMOKE_SCRIPTS S6 section + QUICKSTART §1.2 DEPLOY links |
...
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `af7596419`）

| 项 | 值 |
|----|-----|
| HEAD | `af7596419` — TBOX Phase 65: s6_preflight post-merge chain + DEPLOY_FROM_GITHUB §6/§8 |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **198** |
...
| 项 | 值 |
|----|-----|
| HEAD | `6e6f4e501` — TBOX Phase 62: setup_smoke_env phase16-17 hints, Walkthrough §2.1 VM/SMOKE links |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **194** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `713bb5f14`）

| 项 | 值 |
|----|-----|
| HEAD | `713bb5f14` — TBOX Phase 61: VM §5 align with SMOKE_SCRIPTS, smoke.env.example three-step chain |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **193** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `86b240298`）

| 项 | 值 |
|----|-----|
| HEAD | `86b240298` — TBOX Phase 60: SMOKE_SCRIPTS align with SMOKE_ENV, archive QUICKSTART §1.3 links |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **192** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `66bf43529`）

| 项 | 值 |
|----|-----|
| HEAD | `66bf43529` — TBOX Phase 59: finish doc chain align, SMOKE_ENV QUICKSTART §1.3 cross-refs |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **191** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `d881a9c09`）

| 项 | 值 |
|----|-----|
| HEAD | `d881a9c09` — TBOX Phase 58: QUICKSTART §1.3 align with print_release, handtest manual §5.4 |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **190** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `5658ef4b8`）

| 项 | 值 |
|----|-----|
| HEAD | `5658ef4b8` — TBOX Phase 57: manual §5.4 README step D align, print_release cross-refs |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **189** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `b752e564a`）

| 项 | 值 |
|----|-----|
| HEAD | `b752e564a` — TBOX Phase 56: pre_release --help cross-refs host_check, README step D handtest |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **188** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `8df0c53fa`）

| 项 | 值 |
|----|-----|
| HEAD | `8df0c53fa` — TBOX Phase 55: align host_check hints with web_tbox_check, Walkthrough D handtest |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **187** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `a6f068b9f`）

| 项 | 值 |
|----|-----|
| HEAD | `a6f068b9f` — TBOX Phase 54: web_tbox_check phase16-17 hints, Harness Phase 51-53 summary |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **186** |
...
生成：`bash scripts/tbox_s6_preflight.sh --write-runbook`
### 2026-05-31 快照（`--fetch` 后）（`tbox-deploy` @ `61d1695cf`）

| 项 | 值 |
|----|-----|
| HEAD | `61d1695cf` — TBOX Phase 53: web-tbox README and manual §5.4 align with ENV §6 |
| upstream | `origin/main` @ `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| merge-base | `cd18cfab79578bc0e2797b2614afde44f1e0a411` |
| behind | **0** |
| ahead | **185** |
