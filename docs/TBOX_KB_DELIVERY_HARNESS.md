...
- **PR 轻量门禁（`ubuntu-latest`，路径触发）**：与上条**独立**，在变更相关路径时于 PR 上运行 **`web-tbox.yml`**（前端 typecheck + build）、**`harness-monitor-unit.yml`**（`HarnessMonitor` + `test/adversarial_tests.py` 离线用例）、**`tbox-crawl-common-unit.yml`**（爬取相关 `test/unit_test/common/test_tbox_crawl_*.py`）。索引见 **`docs/TBOX_ENV_AND_VERSIONS.md`** §6。
