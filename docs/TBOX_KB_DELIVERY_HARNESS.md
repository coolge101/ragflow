...
| 2026-05-02 | 新增 **`tbox-task-service-unit.yml`**（`tbox_crawl_task_service` 单测）；**`pyproject.toml`** `filterwarnings` 忽略 xgboost 链上 **pkg_resources** `UserWarning`（置于 blanket `error` 之前） |
