...
| **`tbox-app-routes-unit.yml`** | **`api/apps/tbox_app.py`**、`test/unit_test/api/apps/test_tbox_app_health.py`、**`api/constants.py`**、`pyproject.toml`、`uv.lock` | **`uv sync --group test --frozen`** + **`pytest`**：隔离加载 Blueprint，断言 **`GET /v1/tbox/health`** 与 **`GET /v1/tbox/contract`**（不导入完整 **`api.apps`**，避免 CI 无网拉模型元数据失败） |
