...
| **`tbox-app-routes-unit.yml`** | **`api/apps/tbox_app.py`**、`test/unit_test/api/apps/test_tbox_app_health.py`、**`api/constants.py`**、`pyproject.toml`、`uv.lock` | **`uv sync --group test --frozen`** + **`pytest`**：隔离 Blueprint；**`/health`**、**`/contract`**；mock **`crawl_svc`** + async **`get_request_json`**：**`GET /crawl/tasks`**、鉴权、**`GET .../<id>`** 404；**`POST /crawl/tasks`** 缺 **`name`** 与成功创建 |
