...
| **`tbox-app-routes-unit.yml`** | **`api/apps/tbox_app.py`**、`test/unit_test/api/apps/test_tbox_app_health.py`、**`api/constants.py`**、`pyproject.toml`、`uv.lock` | **`uv sync --group test --frozen`** + **`pytest`**：隔离 Blueprint；**`/health`**、**`/contract`**、**`/me`**、**`/logout`**；mock **`crawl_svc`**：**crawl 任务** **`GET`/`POST`/`PATCH`/`DELETE`**、**`POST …/run`**（成功、**`ValueError`**、**`RuntimeError`** + **`record_worker_tick`**） |
