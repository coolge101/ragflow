...
| **`tbox-app-routes-unit.yml`** | **`api/apps/tbox_app.py`**、`test/unit_test/api/apps/test_tbox_app_health.py`、**`api/constants.py`**、`pyproject.toml`、`uv.lock` | **`uv sync --group test --frozen`** + **`pytest`**：隔离加载 Blueprint（不导入完整 **`api.apps`**）；**`/health`**、**`/contract`**；**`/crawl/tasks`** 在 mock **`crawl_svc`** 下空列表、**`crawl.manage`** 拒绝、**`GET .../crawl/tasks/<id>`** 未找到 |
