...
| **`tbox-app-routes-unit.yml`** | **`api/apps/tbox_app.py`**、**`test/unit_test/api/apps/tbox_app_isolated/**`**、**`api/constants.py`**、`pyproject.toml`、`uv.lock` | **`uv sync --group test --frozen`** + **`pytest`** **`tbox_app_isolated/`** **`-m tbox_app_isolated`**：隔离 Blueprint；**`/health`**、**`/contract`**、**`/me`**、**`/logout`**；mock **`crawl_svc`**：**crawl 任务** **`GET`/`POST`/`PATCH`/`DELETE`**、**`POST …/run`**（成功、**`ValueError`**、**`RuntimeError`** + **`record_worker_tick`**）；marker 在 **`pyproject.toml`** 注册、包 **`conftest.py`** 自动打 **`tbox_app_isolated`** 及子 marker（**`tbox_app_crawl`** / **`tbox_app_session`** / **`tbox_app_public`**，便于本地 **`pytest -m '… and tbox_app_crawl'`**） |
