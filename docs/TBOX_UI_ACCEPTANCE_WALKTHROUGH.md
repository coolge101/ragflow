...
1. 左侧点 **「检索」**。
2. 选择 **知识库**，输入检索词，点 **检索**。
3. **通过标准**：有结果时列表非空；**故意**输入不可能命中的词，应出现 **「无命中」** 黄底说明；点 **「清空条件」** 后输入框与结果应被清掉。
...
2. 浏览列表、尝试「新建 / 编辑 / 删除 / 执行一次」等你环境允许的操作（不要破坏生产数据则用测试环境）。
3. **通过标准**：列表能加载；错误时有重试或明确提示。
...
- **全部通过**：可以明确说「一期控制台验收已全部通过」，再按排期进入二期能力（例如对话/检索多格式导出、审计筛选等，见 `docs/TBOX_PHASE2_PAGE_REQUIREMENTS_MEMO.md`）。
