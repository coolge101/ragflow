...
3. 绑定目标知识库时，下拉应显示 **库名称**（`page_size`≤100）；四类专题库（法规/技术趋势/市场/产品行业）各可建独立任务。
4. 试 **「新建 / 编辑 / 删除 / 执行一次」** 等你环境允许的操作（不要破坏生产数据则用测试环境）。
5. **通过标准**：列表能加载；策略字段保存后再次编辑仍可见；错误时有重试或明确提示。
6. **已知缺口（2026-06-02）**：当前为 **种子 URL + 可选跟链**，**无** 公网搜索发现、**无** 内容级去重、**无** 英文 discover 模板 — 见矩阵 **Phase 67**（DISCOVER + DEDUP + I18N）与 **[`TBOX_5180_HANDTEST_2026-06-02.md`](./TBOX_5180_HANDTEST_2026-06-02.md)**。
