...
7. **引用联动（Phase 16）**：绑定知识库的应用提问后，若回答含 `[ID:0]` 等标记，**点击编号** → 右侧「本轮引用」对应片段**高亮并滚动**；点击侧栏片段可反向高亮编号。
...
1. 左侧点 **「检索」**。
2. 选择 **知识库**，输入检索词，点 **检索**。
3. **通过标准**：有结果时列表非空；**点击某条结果可高亮**（Phase 17）；结果区有 **「导出 Markdown / PDF / Excel / PPT」**；**故意**输入不可能命中的词，应出现 **「无命中」** 黄底说明；点 **「清空条件」** 后输入框与结果应被清掉。
