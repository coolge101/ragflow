...
| DELETE | `/v1/tbox/crawl/sources/<source_id>` | 软删除。 |
| POST | `/v1/tbox/crawl/tasks/<task_id>/import-sources` | 将 catalog 中同 **`topic`** 且 **`enabled=true`** 的 URL 合并进任务 **`seed_urls`**。JSON：`topic`（必填）、`replace`（布尔，true 时以 catalog 替换同 topic 既有种子）。 |
| POST | `/v1/tbox/crawl/tasks/<task_id>/run` | **手动触发一次**与 worker 相同的 **tick**（更新 **`last_run_at`** / **`last_error`**）。默认对 **`seed_urls`** 前 **N** 条做 **轻量 HTTP 探测**（与入库相同：**手动重定向** + **每 hop SSRF + robots**，见 `common/tbox_crawl_ssrf_fetch.probe_url_streaming_cap`）；若已绑定 **`dataset_id`**：**`static_web`** 为 **SSRF GET**（有界体积，**每 hop robots**）；**`rss`** 对 **Feed URL** robots 后再 **`RSSConnector`** 拉条目 **`.txt` 入库**；**`http_api`** 对 endpoint **SSRF GET JSON**，按 **`extra_config`** 展开数组项为 **`.txt` 入库**（见 **§1.4**）。**`extra_config`**：**`tbox_skip_http_probe`** / **`tbox_skip_ingest`** / **`tbox_skip_robots_check`**。不要求 `run_state=ready`。 |
...
**`extra_config` 常用键**（布尔；`PATCH` 传入的 `extra_config` 为**整对象替换**，其它键须自行带回）：**`tbox_skip_http_probe`**、**`tbox_skip_ingest`**、**`tbox_skip_robots_check`**、**`worker_stub_fail`**（tick 最前故意失败，联调用）。**爬取策略**（由 worker / `common/tbox_crawl_strategy.py` 消费；**`web-tbox`** **`/crawl`** 表单写入）：**`tbox_crawl_keywords`** — 字符串数组，入库前过滤页面/Feed 条目正文（任一关键词子串命中则保留；空则不过滤）；**`tbox_crawl_max_depth`** — 非负整数，**`static_web`** 时从种子 BFS 跟随 HTML 链接的最大深度（未设置或 **0** = 仅种子 URL；**>0** = 扩展链接，总量受 **`TBOX_CRAWL_EXPAND_MAX`** 默认 **20** 限制）；**`tbox_crawl_allowed_domains`** — 字符串数组，主机名规则（含子域），过滤种子与扩展链接。**搜索发现（Phase 67–68，`common/tbox_crawl_discover.py`）**：**`tbox_crawl_search_provider`** — `none` \| `tavily` \| **`searxng`**；**`tbox_crawl_search_queries`** — 字符串数组；**`tbox_crawl_search_locale`** — `zh` \| `en` \| `both`；**`tbox_crawl_discover_max_urls`** / **`tbox_crawl_discover_max_queries`** / **`tbox_crawl_discover_max_results_per_query`**；**`tbox_crawl_tavily_depth`** — `basic` \| `advanced`。**Tavily Key 仅 worker 环境**：**`TBOX_CRAWL_TAVILY_API_KEY`** 或 **`TAVILY_API_KEY`**（无 Key → **`DISCOVER_NO_KEY`**）。**SearXNG**：**`TBOX_CRAWL_SEARXNG_BASE_URL`**（无服务 → **`DISCOVER_NO_SEARXNG`**，有种子时降级 seed-only）。**内容质量（Phase 68）**：**`tbox_crawl_url_quality_mode`** — `normal` \| `strict` \| `off`；**`tbox_crawl_extract_main_content`**（bool，默认 true）；**`tbox_crawl_min_extract_chars`**（整数）；**`tbox_crawl_discover_skip_bfs`**（bool，discover URL 默认不 BFS）。tick 摘要可含 **`skipped_low_quality_url`** / **`skipped_low_quality`**。**去重（`tbox_crawl_seen` 表 + `common/tbox_crawl_dedup.py`）**：tick 前 **`url_seen`** 过滤；fetch 后 **`content_sha256`** + **`content_seen`** 跳过重复正文；成功 tick **`last_error`** 可写 **`[tbox:TICK_OK] discovered=N ingested=M skipped_dup_url=…`**。**I18N 抓取**：HTTP **`Accept-Language`** 默认 **`zh-CN,en;q=0.9`**，可用 **`TBOX_CRAWL_ACCEPT_LANGUAGE`** 覆盖。**高级源（§1.4）**：**`tbox_crawl_auth_profile`** — 凭据 profile 名（**仅名称入库**）；**`tbox_crawl_api_items_path`** / **`tbox_crawl_api_content_fields`** / **`tbox_crawl_api_id_field`** — **`http_api`** JSON 解析。**禁止**在 **`extra_config`** 写入 token/password/headers 等密钥（创建/PATCH 校验，见 **`common/tbox_crawl_auth.validate_extra_config_no_secrets`**）。**瞬时 HTTP 重试白名单**（可选）：**`tbox_crawl_retry_statuses`** — 逗号分隔字符串或 **JSON 整数数组**（**100–599**），对该任务**全量替换**白名单（当进程 **`TBOX_CRAWL_RETRY_STATUSES`** 未生效时；与 **`tbox_crawl_retry_extra_statuses`** 二选一优先：**`tbox_crawl_retry_statuses`** 优先，存在且非空解析结果则忽略 **`tbox_crawl_retry_extra_statuses`**）。**`tbox_crawl_retry_extra_statuses`** — 同上格式，与内建默认及 **`TBOX_CRAWL_RETRY_EXTRA_STATUSES`** **并集**（未设置全量替换键/变量时）。见 **`common/tbox_crawl_ssrf_fetch.effective_retry_statuses`**。**`web-tbox`** **`/crawl`**：对上述四键提供勾选，并以 **JSON 文本框** 编辑其余键（默认展示**已剥离**四键；可选 **「JSON 含勾选四键」** 查看/编辑完整对象；完整模式下勾选变更会重合并到文本；**保存**时先解析 JSON 再合并勾选，**勾选优先**覆盖四键）。见 `docs/TBOX_UI_DESIGN_DETAIL.md` §4。
...
| 当前行为 | 写 **`last_run_at`** / **`last_error`**。默认 **HTTP 探测**（`common/tbox_crawl_http_probe.py`）；**`robots.txt`**：`common/tbox_crawl_robots.py`（`urllib.robotparser` + SSRF 安全拉取 **`/robots.txt`**，404 视为允许；拉取/解析失败则**放行**并打日志）。**`extra_config.tbox_skip_robots_check`** 关闭 robots。若 **`dataset_id`** 非空：**`static_web`** / **`rss`** / **`http_api`** 入库路径见 §1.2–§1.4；入库 GET **每个重定向 hop 前**均 `can_fetch`（`common/tbox_crawl_ssrf_fetch.py` 的 **`robots_preflight`**）。**`tbox_skip_http_probe`** / **`tbox_skip_ingest`** 见 §1.2。**`worker_stub_fail`** 在 tick 最前抛错。 |
| robots 节奏 | **`can_fetch`（Disallow/Allow）** + **`OriginFetchThrottler`**（`common/tbox_crawl_origin_throttle.py`）：同一 tick 内对同一 origin 的 **GET** 之间按 **`Crawl-delay`**（`RobotFileParser.crawl_delay`，上限 **`TBOX_CRAWL_MAX_CRAWL_DELAY_SEC`** 默认 **60**）与 **`TBOX_CRAWL_MIN_ORIGIN_INTERVAL`**（默认 **0**）取较大值节流；可与 **`TBOX_CRAWL_SKIP_CRAWL_DELAY`** 关闭 **Crawl-delay** 部分。**`/robots.txt`** 拉取时间计入间隔。**408/429/502/503/504** 与 **520–524**、**525**、**526**、**528**、**529**、**530**（CDN / 托管等边缘或**非标准**码）在抓取层按 **`Retry-After`** 或指数退避重试（参数见下）。**`500`**/**`501`** 等不在 **`effective_retry_statuses(...)`** 白名单内的状态码**不重试**（默认白名单见 **`DEFAULT_RETRY_STATUS_CODES`**，可由 **`TBOX_CRAWL_RETRY_STATUSES`** / **`TBOX_CRAWL_RETRY_EXTRA_STATUSES`** / **`extra_config.tbox_crawl_retry_statuses`** / **`extra_config.tbox_crawl_retry_extra_statuses`** 调整，见 **可调参数**）。**Request-rate** 仍待 **`docs/TBOX_KB_DELIVERY_HARNESS.md` §9.4**。 |
| Docker | 环境变量 **`ENABLE_TBOX_CRAWL_WORKER=1`**，或 **`docker/entrypoint.sh`** 传入 **`--enable-tbox-crawl-worker`**。 |
| 可调参数 | Worker：**`TBOX_CRAWL_WORKER_INTERVAL`**（默认 **30**）、**`TBOX_CRAWL_WORKER_POLL_LIMIT`**（默认 **20**）。探测：**`TBOX_CRAWL_FETCH_PROBE_MAX`**、**`TBOX_CRAWL_FETCH_TIMEOUT`**、**`TBOX_CRAWL_HTTP_USER_AGENT`**、**`TBOX_CRAWL_HTTP_READ_BYTES`**（默认 **8192**）。**robots**：**`TBOX_CRAWL_ROBOTS_TIMEOUT`**（默认 **10**）、**`TBOX_CRAWL_ROBOTS_MAX_BYTES`**（默认 **262144**）。**origin 节流**：**`TBOX_CRAWL_MIN_ORIGIN_INTERVAL`**（秒，默认 **0**）、**`TBOX_CRAWL_MAX_CRAWL_DELAY_SEC`**（默认 **60**）、**`TBOX_CRAWL_SKIP_CRAWL_DELAY`**（真值则忽略 **Crawl-delay**）。**瞬时错误退避**：**`TBOX_CRAWL_RETRY_MAX_ATTEMPTS`**（默认 **2**）、**`TBOX_CRAWL_RETRY_BACKOFF_BASE`**（秒，默认 **1.0**）、**`TBOX_CRAWL_RETRY_BACKOFF_MAX`**（秒，默认 **15**）、**`TBOX_CRAWL_RETRY_AFTER_CAP_SEC`**（秒，默认 **30**）。**重试状态码白名单**：**`TBOX_CRAWL_RETRY_STATUSES`** — 非空则**仅**重试所列 **100–599** 整数（逗号/空格分隔），**完全替换**内建默认与 **`TBOX_CRAWL_RETRY_EXTRA_STATUSES`** / 任务 **`tbox_crawl_retry_*`**；**`TBOX_CRAWL_RETRY_EXTRA_STATUSES`** — 与内建默认**并集**（在未设置全量 **`TBOX_CRAWL_RETRY_STATUSES`** 且任务未设 **`tbox_crawl_retry_statuses`** 时生效）；任务 **`extra_config.tbox_crawl_retry_statuses`** — 全量替换（第二优先级，见 §1.2）。可按状态覆盖退避参数（**`TBOX_CRAWL_RETRY_MAX_ATTEMPTS_<CODE>`**、**`TBOX_CRAWL_RETRY_BACKOFF_BASE_<CODE>`**、**`TBOX_CRAWL_RETRY_AFTER_CAP_SEC_<CODE>`**，**`<CODE>`** 为任意 **100–599** 与 **`TBOX_CRAWL_RETRY_STATUSES`** 中码一致即可）；**`TBOX_CRAWL_RETRY_MAX_ATTEMPTS_<CODE>=0`** 表示对该 **CODE** 不在抓取层重试（不重发 GET）；未设置则回落到全局同名参数）。**static_web** 入库：**`TBOX_CRAWL_INGEST_*`**。**rss** 入库：**`TBOX_CRAWL_RSS_*`**。**http_api** 入库：**`TBOX_CRAWL_API_MAX_ENDPOINTS`**（默认 **3**）、**`TBOX_CRAWL_API_MAX_ITEMS`**（默认 **30**）。**`TBOX_CRAWL_INGEST_TIMEOUT`** 作用于 **RSS Feed HTTP**、**static_web GET** 与 **http_api GET**（回落 **`TBOX_CRAWL_FETCH_TIMEOUT`**）。 |
| 本地开发 | 依赖服务已起后：`export PYTHONPATH=$(pwd)`，执行 **`python rag/svr/tbox_crawl_worker.py`**。 |

### 1.4 爬取凭据与 API 源（Harness §7.4）

与 **`docs/TBOX_KB_DELIVERY_HARNESS.md` §7.4** 一致：**生产密钥不得入库或进 Git**；任务 **`extra_config`** 只存 **profile 名** 与 **API 映射键**，实际 HTTP Header 由 **worker 进程环境** 注入。

| 场景 | 任务字段 | Worker 环境变量 | 说明 |
|------|----------|-----------------|------|
| 需登录 **static_web** / **http_api** | **`extra_config.tbox_crawl_auth_profile`** = `intranet`（示例） | **`TBOX_CRAWL_AUTH_INTRANET_HEADERS`** = JSON 对象，如 `{"Authorization":"Bearer …"}` | **`common/tbox_crawl_auth.build_fetch_headers`** 合并到 SSRF GET。Profile 名：`[A-Za-z0-9][A-Za-z0-9._-]{0,63}`。 |
| **http_api** JSON 拉取 | **`source_type=http_api`**；**`seed_urls`** = API endpoint（GET） | — | **`tbox_crawl_api_items_path`** — 点分路径定位 JSON **数组**（空 = 根须为数组）；**`tbox_crawl_api_content_fields`** — 正文字段列表；**`tbox_crawl_api_id_field`** — 文件名 stem（默认 `id`）。**`ingest_http_api_seeds_into_kb`**。 |

**Compose / 本地**：在 **`docker/.env`** 或 worker 启动环境中设置 Header JSON；**POST/PATCH** 任务会拒绝 **`extra_config`** 中的 token/password/headers 等密钥键。
...
- 禁止在仓库中存放生产密钥；使用环境变量或 Compose secrets（**§1.4 / Harness §7.4**）。
...
| 2026-05-02 | **§1.3**：抓取层瞬时退避增加 **525**、**526**、**530**（**`TBOX_CRAWL_RETRY_*_<CODE>`**）；**robots 节奏** 行注明 **500/501** 等 **5xx** 默认不重试；**`TBOX_KB_DELIVERY_HARNESS.md` §9.4.2** 同步 |
| 2026-05-02 | **§1.3**：抓取层瞬时退避增加 **528**（**`TBOX_CRAWL_RETRY_*_528`**；与 **525/526/530** 同策略）；**`TBOX_KB_DELIVERY_HARNESS.md` §9.4.2** 同步 |
| 2026-05-02 | **§1.3**：抓取层瞬时退避增加 **529**；**可调参数** 明确 **`TBOX_CRAWL_RETRY_MAX_ATTEMPTS_<CODE>=0`** 关闭该状态码重试；**`TBOX_KB_DELIVERY_HARNESS.md` §9.4.2** 同步 |
