...
| Git HEAD | `0af434ace` |
| `tbox_smoke_suite.sh` | pass (via VM 7-step) |
| `tbox_vm_production_acceptance.sh` | pass (7/7) |
| `tbox_web_tbox_check.sh` | pass (via VM) |
| `tbox_login_smoke.sh` | pass (via VM) |
| `tbox_console_bundle_smoke.sh` | pass (via VM) |
| `tbox_permissions_smoke.sh` | pass (via VM) |
| `tbox_dual_account_check.sh` | not configured (optional) |
| §3 内网与双账号 A–D | ☑ 手测 2026-06-02 |
| §4 产品动线 Walkthrough | ☑ 手测 2026-06-02 |
